...
if not "%new_version%"=="" if not "%new_version%"=="%current_version%" (
    echo Available new version: %new_version% (current: %current_version%)
    echo Update? (Y/N)
    choice /c YN /n
    if errorlevel 2 (
...
set "cmd=!input!"
if /i "!cmd!"=="/exit" color 00 && color 0f && cls && exit /b
if /i "!cmd!"=="/back" (
    if not "!prev_dir!"=="" (
        set "temp=!cd!"
        cd /d "!prev_dir!" 2>nul
...
if /i "!cmd:~0,8!"=="/delete " (
    ...
    if /i "!subcmd!"=="folder" (
        call :delete_folder "!arg!"
    ) else (
        call :delete_file "!args!"
    )
    goto show
)
if /i "!cmd:~0,9!"=="/download " (
    set "rest=!cmd:~9!"
    for /f "tokens=1,*" %%a in ("!rest!") do (
        set "url=%%a"
        set "fname=%%b"
    )
    call :download_file "!url!" "!fname!"
    goto show
)
...
if /i "!type!"=="file" (
    if not "!name!"=="" (
        echo.>>"!name!" 2>nul
        if errorlevel 1 (
            echo Failed to create file "!name!"
        ) else (
...
echo /back - go to previous directory
echo /create file ^<filename^> - create a new file
echo /create folder ^<foldername^> - create a new folder
echo /open ^<filename^> - open file with default program
echo /edit [^<filename^>] - edit file with Notepad (waits for close)
echo /delete ^<filename^> - delete a file
echo /delete folder ^<foldername^> - delete a folder and all its contents
echo /download ^<url^> [^<filename^>] - download a file from the Internet
echo /cd ^<drive^>: - switch to another drive
echo /help - show this help
echo /exit - quit
echo ^<foldername^> - just type folder name to go there
pause
goto :eof
...
if not "!drive!"=="" (
    cd /d "!drive!" 2>nul
    if errorlevel 1 (
        echo Cannot change drive to "!drive!"
    ) else (
        set "prev_dir=!cd!"
    )
) else (
...
set "folder=%~1"
if "!folder!"=="" (
    echo Usage: /delete folder foldername
    goto :eof
)
if not exist "!folder!" (
...
if errorlevel 1 (
    echo Failed to delete folder "!folder!"
) else (
...
set "fname=%~2"
if "!url!"=="" (
    echo Usage: /download ^<url^> [filename]
    goto :eof
)
if "!fname!"=="" set "fname=downloaded_file"
echo Downloading from !url! to "!fname!" ...
powershell -Command "Invoke-WebRequest -Uri '!url!' -OutFile '!fname!'" 2>nul
if errorlevel 1 (
